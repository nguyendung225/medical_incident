...
const TablePagination: FC<TablePaginationProps> = (props) => {
  ...
  function fetchPageNumbers() {
    const totalNumbers = pageNeighbours * 2 + 1; // Số trang cần hiển thị (1 2 ... 4 5 6 ... 7)
  ...
  return (
    <div
      className={clsx(
        "flex border pagination-container justify-content-end gap-2 border-top align-items-center bg-white",
        className
      )}
    >
      <Pagination className="pagination justify-content-center align-items-center my-2">
      ...
      <div className="d-flex justify-content-centerr gap-2 align-items-center ps-0 pe-0">
        <Form.Select
          size="sm"
          value={rowsPerPage}
          onChange={(e) => {
          ...
          className="w-100px select"
        >
          {rowsForPage.map((item) => (
            <option key={item} value={item}>
              <div className="fs-14px">{item}/trang</div>
            </option>
          ))}
        </Form.Select>
      </div>
